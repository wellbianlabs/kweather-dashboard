  ...
  return (
    <div className="rounded-xl bg-white p-4 shadow-sm border border-slate-200">
      <div className="mb-2 flex items-center justify-between">
        <h3 className="font-semibold text-slate-700">현재 외부 날씨 (케이웨더 실측)</h3>
      ...
      {cw && cw.available ? (
        <>
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
            <Cell label="외부 기온" value={cw.outdoor_temp} unit="℃" />
            <Cell label="외부 체감온도" value={cw.outdoor_feels} unit="℃" />
            <Cell label="외부 습도" value={cw.outdoor_humidity} unit="%" />
          ...
          {cw.delta != null && (
            <div className={`mt-3 rounded-lg px-3 py-2 text-sm ${
              ...
              {cw.enclosed_alert ? "⚠️ " : "· "}
              현장 체감온도가 외부 기온보다 <b>{cw.delta > 0 ? `${cw.delta}℃ 높음` : `${Math.abs(cw.delta)}℃ 낮음`}</b>
        ...
        <p className="py-4 text-center text-sm text-slate-400">
          {cw?.message || "현재 외부 날씨를 불러올 수 없습니다."}
        </p>
      )}
    </div>
  );
